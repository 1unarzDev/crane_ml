...
export CRANE_SCENE="${CRANE_SCENE:-Land Vehicle Validation}"
export CRANE_NOGRAPHICS=1
export CRANE_NAV2_PROFILE=train-cpu
export CRANE_NAV2_PARAMS="${CRANE_NAV2_PARAMS:-${root_dir}/Tools/Performance/nav2_land_fixture.yaml}"
export CRANE_NAV2_COMMAND_FLAG="${CRANE_NAV2_COMMAND_FLAG:---crane-ros-ackermann-cmd-vel}"
...
export CRANE_NAV2_UNITY_EXTRA_ARGS="--crane-land-nav2 --crane-ros-nav-child-frames ${lidar_frame} --crane-land-evaluator-output ${result_root}/worker-${worker_id}/land-evaluator-truth.json ${CRANE_NAV2_UNITY_EXTRA_ARGS:-}"
