...
using System.IO;
using UnityEngine;
using static Unity.Robotics.ROSTCPConnector.MessageGeneration.MsgAutoGenUtilities;
    ...
    public class MessageParser
    {
        ...
        public void Parse()
        {
            ...
            using (StreamWriter writer = new StreamWriter(outFilePath, false))
            {
                ...
                writer.Write(
                    ONE_TAB + "[Serializable]\n" +
                    ONE_TAB + "public class " + className + " : Message\n" +
                    ONE_TAB + "{\n"
                    );
                ...
                writer.Write(GenerateDeserializingConstructor());
        ...
        private string ConstantDeclaration(string type)
        {
            ...
            return ParseLiteral(type, declaration);
        }

        private string DefaultValueDeclaration(string type)
        {
            string declaration = MatchByType(MessageTokenType.DefaultValueDeclaration);
            return ParseLiteral(type, declaration);
        }

        string ParseLiteral(string type, string declaration)
        {
        ...
        private string GenerateDefaultValueConstructor()
        {
            ...
            {
                if (!constants.Contains(identifier) && !defaultValues.Contains(identifier))
                {
        ...
        private string GenerateParameterizedConstructor()
        {
            ...
            foreach (string identifier in symbolTable.Keys)
            {
                if (!constants.Contains(identifier))
                {
                    ...
                    assignments += THREE_TABS + "this." + identifier + " = " + identifier + ";\n";
                }
            }
        ...
        private string GenerateDeserializingConstructor()
        {
            string constructor = "";
            ...
            foreach (string identifier in symbolTable.Keys)
            {
                if (!constants.Contains(identifier))
                {
                    ...
                    if (arraySizes.ContainsKey(identifier))
                    {
                        ...
                        string arrayElementType = type.Substring(0, type.Length - 2);

                        string lengthString = (arraySize == 0) ? "deserializer.ReadLength()" : arraySize.ToString();

                        if (arrayElementType == "string")
                            assignments += THREE_TABS + $"deserializer.Read(out this.{identifier}, {lengthString});\n";
                        else if (MsgAutoGenUtilities.nonMessageTypes.Contains(arrayElementType))
                            assignments += THREE_TABS + $"deserializer.Read(out this.{identifier}, sizeof({arrayElementType}), {lengthString});\n";
                        else
                            assignments += THREE_TABS + $"deserializer.Read(out this.{identifier}, {arrayElementType}.Deserialize, {lengthString});\n";
                    }
                    else if (MsgAutoGenUtilities.nonMessageTypes.Contains(type))
                    {
            ...
            constructor += TWO_TABS + $"private {className}(MessageDeserializer deserializer)\n";
            constructor += TWO_TABS + "{\n";
            constructor += assignments;
            constructor += TWO_TABS + "}\n\n";
        ...
        {
            string function = "";
            function += MsgAutoGenUtilities.TWO_TABS + "public override void DeserializeFrom(MessageDeserializer deserializer)\n";
            ...
            foreach (string identifier in symbolTable.Keys)
            {
                if (!constants.Contains(identifier))
                {
                    ...
                    if (arraySizes.ContainsKey(identifier))
                    {
                        ...
                        if (arraySize == 0)
                        {
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"deserializer.Read(ref this.{identifier}, deserializer.ReadLength());\n";
                        }
                        else
                        {
                            // fixed length array, we already know the length
                            function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"deserializer.Read(ref this.{identifier}, {arraySize});\n";
                        }
                    }
                    else if (MsgAutoGenUtilities.nonMessageTypes.Contains(type))
                    {
                        function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"deserializer.Read(out this.{identifier});\n";
                    }
                    else
                    {
                        function += MsgAutoGenUtilities.TWO_TABS + MsgAutoGenUtilities.ONE_TAB + $"deserializer.Read(this.{identifier});\n";
                    }
                }
            }
        ...
        private string GenerateSerializationStatements()
        {
            ...
            foreach (string identifier in symbolTable.Keys)
            {
                ...
                {
                    int arraySize;
                    if (arraySizes.TryGetValue(identifier, out arraySize) && arraySize == 0)
                    {
                        // variable length array, serialize the length first
                        function += THREE_TABS + $"serializer.WriteLength(this.{identifier});\n";
                    }
        ...
        {
            string toString = "";
            toString += TWO_TABS + "public override string ToString()\n";
            toString += TWO_TABS + "{\n";
            toString += THREE_TABS + "return \"" + className + ": \" +\n";
