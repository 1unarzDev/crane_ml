...
{
    // Modified by CRANE: synchronize topic snapshots/get-or-create with the connection thread.
    public class ROSConnection : MonoBehaviour
    {
        ...
        public RosTopicState GetOrCreateTopic(string topic, string rosMessageName, bool isService = false)
        {
            ...
            lock (m_Topics)
            {
                if (m_Topics.TryGetValue(topic, out RosTopicState state))
                {
                ...
                result = AddTopic(topic, rosMessageName, isService);
            }
