...
{
    // Modified by CRANE: track one publisher registration per TCP connection.
    public class RosTopicState
    {
        ...
        public void RegisterPublisher(int queueSize, bool latch)
        {
            lock (this)
            {
                ...
                CreateMessageSender(queueSize);
                // A registration queued before the socket handshake is still valid for the
                // first connection. Mark it here so OnConnectionEstablished does not also send
                // an immediate duplicate. OnConnectionLost clears the flag for reconnect.
        ...
        internal void OnConnectionEstablished(NetworkStream stream)
        {
            lock (this)
            {
                ...
                {
                    // Register before sending topic payloads on a reconnect. Initial
                    // registrations may already be queued by RegisterPublisher.
