...
5. A live observation-derived ROS loop now returns `TwistStamped` and validates acquisition,
   ...
   0.535 m while all target sensors remain valid. Its voxel local costmap consumes `/points` and
   publishes occupied/inflated cells; pairing `Twist` with the newest delivered odometry
